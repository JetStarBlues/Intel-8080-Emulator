...
START:  LXI  SP,STACK                   ;*** COLD START ***       ; 0, 1, 2
        ...
        JMP  INIT                                                 ; 5, 6, 7
;
        XTHL                            ;*** TSTC OR RST 1 ***    ; 8
        RST  5                          ;IGNORE BLANKS AND        ; 9
        CMP  M                          ;TEST CHARACTER           ; 10
        JMP  TC1                        ;REST OF THIS IS AT TC1   ; 11, 12, 13
;
CRLF:   MVI  A,CR                       ;*** CRLF ***             ; 14, 15
;
        ; PUSH PSW                        ;*** OUTC OR RST 2 ***    ; 16
        ; LDA  OCSW                       ;PRINT CHARACTER ONLY     ; 17, 18, 19
        ; ORA  A                          ;IF OCSW SWITCH IS ON     ; 20
        ; JMP  OC2                        ;REST OF THIS IS AT OC2   ; 21, 22, 23
        JMP  OC2                        ;REST OF THIS IS AT OC2   ; 16, 17, 18
        NOP                                                       ; 19
        NOP                                                       ; 20
        NOP                                                       ; 21
        NOP                                                       ; 22
        NOP                                                       ; 23
;
        CALL EXPR2                      ;*** EXPR OR RST 3 ***    ; 24, 25, 26
        PUSH H                          ;EVALUATE AN EXPRESSION   ; 27
        JMP  EXPR1                      ;REST OF IT AT EXPR1      ; 28, 29, 30
        DB   'W'                                                  ; 31
;
        MOV  A,H                        ;*** COMP OR RST 4 ***    ; 32
        CMP  D                          ;COMPARE HL WITH DE       ; 33
        RNZ                             ;RETURN CORRECT C AND     ; 34
        MOV  A,L                        ;Z FLAGS                  ; 35
        CMP  E                          ;BUT OLD A IS LOST        ; 36
        RET                                                       ; 37
        DB   "AN"                                                 ; 38, 39
;
SS1:    LDAX D                          ;*** IGNBLK/RST 5 ***     ; 40
        CPI  20H                        ;IGNORE BLANKS            ; 41, 42
        RNZ                             ;IN TEXT (WHERE DE->)     ; 43
        INX  D                          ;AND RETURN THE FIRST     ; 44
        JMP  SS1                        ;NON-BLANK CHAR. IN A     ; 45, 46, 47
;
...
;
INIT:   ; STA  OCSW
        ; MVI  A,0CFH                     ; UART CONFIG
        ; OUT  0FBH
        ; MVI  A,27H                      ; UART CONFIG
        ; OUT  0FBH
        ; MVI  D,19H
PATLOP:
        ; CALL CRLF
...
OCSW:   DS   1                          ;SWITCH FOR OUTPUT       ; 1000
CURRNT: DS   2                          ;POINTS TO CURRENT LINE  ; 1001
STKGOS: DS   2                          ;SAVES SP IN 'GOSUB'     ; 1002, 1003
VARNXT: DS   2                          ;TEMP STORAGE            ; 1004, 1005
STKINP: DS   2                          ;SAVES SP IN 'INPUT'     ; 1006, 1007
LOPVAR: DS   2                          ;'FOR' LOOP SAVE AREA    ; 1008, 1009
LOPINC: DS   2                          ;INCREMENT               ; 100A, 100B
LOPLMT: DS   2                          ;LIMIT                   ; 100C, 100D
LOPLN:  DS   2                          ;LINE NUMBER             ; 100E, 100F
LOPPT:  DS   2                          ;TEXT POINTER            ; 1010, 1011
